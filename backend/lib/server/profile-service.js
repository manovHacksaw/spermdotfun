...
function normalizeWallet(input) {
  try {
    return ethers.getAddress(String(input))
  } catch {
    return null
  }
}
...
function parseSignatureString(signature) {
...
function createProfileService({
  ...
  async function verifyAuthChallenge({ wallet, nonce, signature }) {
    ...
    const signatureStr = parseSignatureString(signature)
    if (!signatureStr) {
    ...
    try {
      recoveredAddress = ethers.verifyMessage(nonceRow.message, signatureStr)
    } catch {
      throw Object.assign(new Error('Signature verification failed'), { code: 'INVALID_SIGNATURE' })
    }

    const isValidSig = recoveredAddress.toLowerCase() === normalizedWallet.toLowerCase()
