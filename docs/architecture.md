...
This document provides a high-level overview of the SPRMFUN project architecture.  It explains the major runtime processes, component interactions, and external services.  For lower‑level technical details (stack, module map, protocol, build pipeline) see [Technical Architecture](technical-architecture.md).
...
A Solidity program deployed on the Avalanche C‑Chain. All token custody, bet lifecycle, and payout arithmetic happen on-chain. The server acts as the trusted **authority** that posts VRF results and resolves bets.
