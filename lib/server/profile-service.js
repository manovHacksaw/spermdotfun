...
// remove bs58 and ed25519; we no longer use Solana keypairs or base58
...
function normalizeWallet(input) {
  ...
  // expect a 0x-prefixed 40‑hex address
  if (/^0x[0-9a-f]{40}$/.test(w)) return w
  return null
}
...
function decodeBetResolvedEventFromRaw(raw) {
  ...
  // last 20 bytes of the 32-byte field contain the address
  const addrBuf = userBytes.slice(12)
  const sourceWallet = '0x' + addrBuf.toString('hex')
...
function createProfileService({
  ...
  async function verifyAuthChallenge({ wallet, nonce, signature }) {
    ...
    // ethers expects a hex signature string
    let recovered
    try {
      recovered = ethers.utils.verifyMessage(nonceRow.message, signature)
    } catch {
